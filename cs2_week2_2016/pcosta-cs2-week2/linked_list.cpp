 ...
 * the linked list. This is potentially problematic because if memory is 
 * not freed, then eventually you could run out of it. It is also just bad 
 * practice and inefficient to not free memory.
 */
...
List::~List()
{
    Node *temp = head;
    Node *prev = head;
    // Loops through all nodes except the last, freeing each one.
